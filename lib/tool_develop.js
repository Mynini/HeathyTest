...
(function(window, undefined) {
    ...
    Fre.extend({
        ...
        type: function(obj) {
            ...
            return typeof obj === "object" || typeof obj === "function" ?
                classType[CT_string(obj)] || "object" : typeof obj;
    ...
    Fre.fn.extend({
        find: function(selector) {
            var self = this;
            var arr = Fre.find(selector);
            Fre.each(arr.objArr, function(i, item) {
                self[i] = item;
            })
    ...
    (function(window, undefined) {
        ...
        function Sizzle(selector, context) {
            ...
            if (claExpr.test(selector)) {
                ...
                returnArr.name = selector.slice(1)
                returnArr.objArr = Sizzle.getClass(name, context);

                //Handle: $("div")
            } else if (tagClaExpr.test(selector)) {

                returnArr.name = selector;
                returnArr.objArr = context.getElementsByTagName(selector);
                ...
                //handle: 最终处理$(".div||div [name=value]")
                    ...
                    for (var i = 0; i < arr.length; i++) {
                        ...
                        if (arr[i].getAttribute(name) && arr[i].getAttribute(name) == value) {
                            ...
                            returnArr.objArr.push(arr[i])
                        }

                    }

                    if (returnArr.objArr.length == 0) {
